...
import { FaEdit, FaEye, FaTrash, FaCircle } from "react-icons/fa";
  ...
  const columns = [
    {
      ...
      render: (value, row) => (
        <div>
          <p className="font-medium text-gray-800">{value || "Untitled"}</p>
    ...
    },
    {
      ...
      render: (_, row) => (
        <div className="flex justify-end gap-2">
          {/* VIEW */}
          <button
            onClick={() => router.push(`/services/${row.slug}`)}
          ...
          <button
            onClick={() =>
              router.push(`/administrator/services/edit/${row._id}`)
            }
            className="p-2 rounded-md hover:bg-gray-100 transition"
          >
